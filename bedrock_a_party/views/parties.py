...
def loaded_parties():
    ...
    result = jsonify({'loaded_parties':_PARTY_NUMBER})
    return result
...
def get_foodlist(id):
    ...
    if 'GET' == request.method:
        food_list = _LOADED_PARTIES[id].get_food_list().serialize();
        result = jsonify({'foodlist': food_list})
...
def edit_foodlist(id, user, item):
    ...
    if 'POST' == request.method:
        try:
            result = result.add_to_food_list(item,user)
